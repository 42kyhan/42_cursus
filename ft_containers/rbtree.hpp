...
namespace ft {
    ...
    struct TreeNode {
        ...
        TreeNode() : _parent(nullptr), _left(nullptr), _right(nullptr), _value(value_type()), _is_black(bool()) {}
        TreeNode(const value_type& value) : _parent(nullptr), _left(nullptr), _right(nullptr), _value(value), _is_black(bool()) {}
    ...
    bool is_left_child( NodePtr ptr) {
        return (ptr == ptr->_parent->_left);
    }

    template <typename NodePtr>
    bool is_right_child( NodePtr ptr) {
        return (ptr == ptr->_parent->_right);
    }

    template <typename NodePtr>
    bool is_black_color( NodePtr ptr) {
        return (ptr->_is_black);
    }

    template <typename NodePtr>
    bool is_red_color(NodePtr ptr) {
        return (!ptr->_is_black);
    }

    template <typename NodePtr>
    NodePtr get_min_node(NodePtr ptr) {
    ...
    template <typename NodePtr>
    NodePtr get_max_node(NodePtr ptr) {
    ...
    template <typename NodePtr>
    NodePtr get_next_node(NodePtr ptr) {
        if (ptr->_right != nullptr)
            return (get_min_node(ptr->_right));
    ...
    template <typename NodePtr>
    NodePtr get_prev_node(NodePtr ptr) {
        if (ptr->_left != nullptr)
            return (get_max_node(ptr->_left));
    ...
    bool is_equal(T& t, U& u, Comp comp) {      
    ...
    public:
        tree_iterator() : _current(nullptr), _nil(nullptr) {}
        tree_iterator(node_pointer curr, node_pointer nil) : _current(curr), _nil(nil) {}
        ...
        tree_iterator& operator++() {
            _current = get_next_node(_current);
            return (*this);
        }
        ...
        tree_iterator& operator--() {
            _current = get_prev_node(_current);
            return (*this);
        }
    ...
    template <typename Tp, class Key, class Comp, class Allocator>
    class rbtree {
    public:
    ...
    public:
        rbtree(const comp_type& comp, const allocator_type& alloc) : _comp(comp), _alloc(alloc), _size(size_type()) {
        ...
        size_type erase(const key_type& value) {
            iterator pos(find_internal(value), _nil);
            if (pos == end())
                return (0);
            if (pos == begin()) {
                iterator tmp(pos);
    ...
    private:
        ...
        node_pointer search_parent(const value_type& value, node_pointer position = nullptr) {
            if (position && position != _end) {
                if (_comp(value, position->_value) && position->_left == nullptr) {
                    iterator prev = iterator(position, _nil);
                    if (prev == begin() || _comp(*--prev, value))
                        return (position);
                }
                else if (position->_right == nullptr) {
        ...
        void remove_internal(node_pointer ptr) {
            ...
            if (ptr->_left == _nil) {
                ...
                fixup_node = get_min_node(ptr->_right);
